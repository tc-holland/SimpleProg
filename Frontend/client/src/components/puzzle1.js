...
export default function App() {
  ...
  // create a stable color map for each word so each word keeps the same color
  // across renders. We use useMemo so it's created once per component lifecycle.
  const colorMap = useMemo(() => {
    const map = {};
    const randomColor = () => {
      // generate pleasant HSL colors with good saturation and lightness
      const h = Math.floor(Math.random() * 360);
      const s = 65 + Math.floor(Math.random() * 15); // 65-80%
      const l = 45 + Math.floor(Math.random() * 7); // 45-52%
      return `hsl(${h} ${s}% ${l}%)`;
    };
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="bg">
        <div className="stack">
          ...
          <p className="subtitle">Drag the words below into the correct blank spaces</p>
                ...
                <div key={sentence.id} className="s1">
                  {sentence.parts.map((part, index) => {
                    if (typeof part === "string") {
                      ...
                      // contains literal backslash-escaped characters, then render with
                      // a CSS class that preserves whitespace and tabs.
